			...
			for(type : reachableTypes){
				...
				if(!success) {
					...
					for(receiverMatch : receiverMatches){
						if(firstReceiver === null){
							if(!suppliedWithAction.contains(receiverMatch.transition)){
								firstReceiver = receiverMatch.transition as Transition
								firstSM = receiverMatch.SM as StateMachine
							}
						} else if(secondReceiver === null && receiverMatch.SM != firstSM){
							if(!suppliedWithAction.contains(receiverMatch.transition)){
								secondReceiver = receiverMatch.transition as Transition
							}					
						}
					}
					if(!suppliedWithAction.contains(senderTransition) && firstReceiver !== null /* && !suppliedWithAction.contains(firstReceiver) <- this condition is implied by the firstReceiver !=null*/){
						...
						success = true
						suppliedWithAction.add(senderTransition as Transition) 
						// Create send action
						val senderAction = SEND_METHOD_NAME + "(" + (type as ApplicationType).identifier + ", "+ index + ")"
						debug(senderAction)
						operations.add(new ActionGenerationOperation(senderAction, senderTransition as Transition));
