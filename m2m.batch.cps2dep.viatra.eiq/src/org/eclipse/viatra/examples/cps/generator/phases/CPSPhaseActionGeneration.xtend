							...
							if(appClass.probabilityOfSendAction.randBooleanWithPercentageOfTrue(fragment.random)){
								...
								if(!possibleTypeList.empty) {
    								...
    								if(targetAppType !== null){
    									val action = SEND_METHOD_NAME + "(" + (targetAppType as ApplicationType).identifier + ", "+ signalNumber + ")";
    									debug(action)
    									operations.add(new ActionGenerationOperation(action, transition as Transition));
    								}else{
